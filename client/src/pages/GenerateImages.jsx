...
const GenerateImages = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post('https://quick-ai-server-7fzzdc43e-afzaal-hassans-projects.vercel.app/api/ai/generate-image', { prompt, publish },
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className="my-6 flex items-center gap-2">
          <label className="relative cursor-pointer">
            <input type="checkbox" onChange={(e) => setPublish(e.target.check)}
              checked={publish} className="sr-only peer" />
            <div className="w-9 h-5 bg-slate-300 rounded-full
                 peer-checked:bg-green-500 transitions"></div>
            <span className='absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition-full
